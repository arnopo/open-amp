...
#define MAX_NB_TX_BUFF  10  /* should be higher than the number Tx buffers used for rpmsg */
...
int app(struct rpmsg_device *rdev, void *priv)
{
	...
	void *buff_list[RPMSG_RECYCLER_SIZE];
	...
	i = 0;
	while (i < RPMSG_RECYCLER_SIZE + 1)
		buff_list[i++] = rpmsg_get_tx_payload_buffer(&lept, &max_size, 1);

	LPRINTF(" 2 - Release the unused Tx buffer\r\n");

	/* test the RPMSG_ERR_NO_MEM return error of rpmsg_release_tx_buffer */
	i--;
	while (i >= 0) {
		ret = rpmsg_release_tx_buffer(&lept, buff_list[i]);
		if (ret != RPMSG_SUCCESS) {
			if (ret != RPMSG_ERR_NO_MEM) {
				LPERROR("Failed to release TX buffer.\r\n");
				return -1;
			}
			if (i > 1) {
				LPERROR("TX buffer recycler should not be full.\r\n");
				return -1;
			}
			LPRINTF(" TX buffer recycler full well detected\r\n");
			break;
		}
		i--;
	}
	/*
	 * One TX buffer as not been released, keep it as lost buffer to ensure correct
	...
	while (1) {
		...
		while (rpmsg_list) {
			...
			rpmsg_release_rx_buffer(rpmsg_list->ept,
						rpmsg_list->data);
			if (!(i++ % 13)) {
				buff_list[0] = rpmsg_get_tx_payload_buffer(rpmsg_list->ept,
									   &max_size, 1);
				if (!buff_list[0]) {
					LPERROR("Failed to get payload...\r\n");
					break;
				}
				ret = rpmsg_release_tx_buffer(rpmsg_list->ept, buff_list[0]);
				if (ret < 0) {
					LPERROR("failed to release TX buffer...\r\n");
					break;
				}
