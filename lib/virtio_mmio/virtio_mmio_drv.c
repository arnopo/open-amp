					      ...
					      const char *vq_name)
{
	...
	struct virtqueue *vqueue = NULL;
	bool vq_allocated = false;
	int offset;
	struct virtio_mmio_device *vmdev = metal_container_of(vdev,
							      struct virtio_mmio_device, vdev);
	...
	if (!vqueue) {
		/* Preallocated buffer before vring */
		...
		vqueue = vring_info->vq;
		vq_allocated = true;
	} else {
		/* Preallocated virtqueues; vrings can be already declared via VIRTQUEUE_DECLARE */
		/* Check if vrings are already configured */
		if (vqueue->vq_nentries != 0 && vqueue->vq_nentries == vqueue->vq_free_cnt &&
		    vqueue->vq_ring.desc) {
			vring_info->info.vaddr = vqueue->vq_ring.desc;
			vring_info->vq = vqueue;
		}
		vring_info->info.num_descs = vqueue->vq_nentries;
	}
	...
	if (virtqueue_create(vdev, idx, vq_name, vring_alloc_info, (void (*)(struct virtqueue *))cb,
		...
		if (vq_allocated && (vring_info->vq != NULL))
			metal_free_memory(vring_info->vq);
		return NULL;
	}
	vdev->role = role_bk;
	vqueue->priv = cb_arg;
	virtqueue_set_shmem_io(vqueue, vmdev->shm_io);

	if (vdev->id.version == 0x2) {
		/* SCMI for now */
		vqueue = virtio_mmio_set_vq(vdev, vring_info->vq);
	} else {
		virtio_mmio_write32(vdev, VIRTIO_MMIO_DRIVER_FEATURES_SEL, idx);
		maxq = virtio_mmio_read32(vdev, VIRTIO_MMIO_QUEUE_NUM_MAX);
		VIRTIO_ASSERT((maxq != 0),
			      "VIRTIO_MMIO_QUEUE_NUM_MAX cannot be 0");
		VIRTIO_ASSERT((maxq >= vqueue->vq_nentries),
			      "VIRTIO_MMIO_QUEUE_NUM_MAX must be greater than vqueue->vq_nentries");
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_NUM, vqueue->vq_nentries);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_ALIGN, 4096);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_PFN,
				    ((uintptr_t)metal_io_virt_to_phys(vqueue->shm_io,
				    (char *)vqueue->vq_ring.desc)) / 4096);
	}

	vdev->vrings_info[vdev->vrings_num].vq = vqueue;
	vdev->vrings_num++;
