...
#define VIRTIO_MSG_QUEUE_MAX_SIZE  0x10
			       ...
			       struct virtio_msg_virtio_msg *msg)
{
	...
	ret = metal_io_block_write(&devbus->msg_io, 0, msg, (int)sizeof(*msg));
	if (ret < 0)
		return ret;
...
static void virtio_msg_bus_vq_notify(struct virtqueue *vq)
{
	...
	msg.event_used.vq_idx = vmdev->bus_id;
				     ...
				     struct virtio_msg_virtio_msg *msg)
{
	...
	alloc_info->vaddr = (void *)vq_cfg->driver_addr;
	...
	vq->vq_ring.desc = (struct vring_desc *)vq_cfg->descriptor_addr;
	vq->vq_ring.avail = (struct vring_avail *)vq_cfg->driver_addr;
	vq->vq_ring.used = (struct vring_used *)vq_cfg->device_addr;
...
int virtio_msg_bus_receive(struct virtio_msg_bus_device *devbus)
{
	...
	metal_io_block_read(&devbus->msg_io, 0, &msg, sizeof(msg));
	...
	case VIRTIO_MSG_SET_FEATURES:
		ret = virtio_msg_set_dev_feats(vmdev, &msg);
		if (ret)
			return ret;
	break;
	case VIRTIO_MSG_GET_VQUEUE:
		msg.get_vq_resp.max_size = VIRTIO_MSG_QUEUE_MAX_SIZE;
	break;
			...
			const struct virtio_msg_bus_cfg *bus_cfg, void *priv)
{
	...
	if (bus_cfg->msg_size < VIRTIO_MSG_MAX_SIZE)
		return -EINVAL;
