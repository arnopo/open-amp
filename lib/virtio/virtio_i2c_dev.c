...
{
	return virtio_device_get_status(dev->vdev) == VIRTIO_CONFIG_STATUS_READY;
}
					...
					void *callback_arg)
{
	...
	int ret = dev->vdev->func->create_virtqueues(dev->vdev, NULL, 1, &"RX", &callback);
	 ...
	 */
	dev->vdev->vrings_info[0].vq->cb_arg = callback_arg;
	return 0;
}
...
int virtio_i2c_handle_avail(struct virtio_i2c_dev *dev)
{
	...
	while (virtqueue_get_desc_size(vq)) {
		...
		read = isSet(VIRTIO_I2C_MSG_READ, header->flags);
